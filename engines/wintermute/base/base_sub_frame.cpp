...
bool BaseSubFrame::draw(int x, int y, BaseObject *registerOwner, float zoomX, float zoomY, bool precise, uint32 alpha, float rotate, TSpriteBlendMode blendMode) {
	...
	if (rotate != 0.0f) {
		/* 
		 * We need some offsets now. The new origin is: old origin + boxOffset + hotspot offset.
		 * boxOffset is the offset caused by the origin being shifted further down and right 
		 ...
		 *
		 * hotspotOffset keeps the rotation centered around the hotspot, i.e. the hotspot
		 * stays at the same coordinates.
		 */
		Common::Point boxOffset, rotatedHotspot, hotspotOffset, newOrigin;
		 ...
		 */
		rotatedHotspot = OffsetTools::rotatePoint(hotspot, rotate); 
		/* 
		 * Now we compensate for zoom 
		 */
		hotspotOffset.x = rotatedHotspot.x * zoomX / 100;
		hotspotOffset.y = rotatedHotspot.y * zoomY / 100;
		/*
		 * We compute the offset due to the padding
		 */
		boxOffset = OffsetTools::computeBoxOffset(getRect(), rotate, zoomX, zoomY); 
		/*
		 * It all comes together: 
		*/
		newOrigin = origin - hotspotOffset  + boxOffset;
		res = _surface->displayTransform(newOrigin.x, newOrigin.y, _hotspotX, _hotspotY, getRect(), zoomX, zoomY, alpha, rotate, blendMode, _mirrorX, _mirrorY);
	} else {
		if (zoomX == 100 && zoomY == 100) {
